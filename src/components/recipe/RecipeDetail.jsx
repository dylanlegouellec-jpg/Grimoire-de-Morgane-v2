...
export default function RecipeDetail({ recipe, onClose, onCook, onEdit, shareText, showToast }) {
  ...
  const nutri = useNutriscore(recipe ? rawIngredients : null);

  // Bloque le scroll (et le rebond tactile) du fond de page tant que la
  // fiche recette est ouverte, pour empêcher l'arrière-plan de défiler
  // en même temps que le panneau glissé vers le bas. Restauré à la
  // fermeture, y compris en cas de démontage brutal (changement d'onglet…).
  useEffect(() => {
